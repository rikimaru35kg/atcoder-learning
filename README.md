...
- その為、f(g)はΣf(i*g) (i>=2)を引いたものとして考えると重複なく数えられる
- この時、f(g)はgの降順で求めていけば順次求まる
### 例題
...
- 最初に完全グラフとなる部分集合を求める。すなわちDP[s]=1となる部分集合を試す
- sの一つの頂点から他頂点へ辺が貼ってあり、かつ、他頂点のDPが1以下の時、DP[s]=1
...
## 蛙型DP
- 蛙が1~Kマスジャンプできるとき、最後のマスに到達する場合の数を求める問題
- 問題設定によっては蛙型であることを見抜くのが難しい
