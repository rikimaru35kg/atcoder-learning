...
- なお、重複要素削除はsetを用い、それをベクターに変換してからソートしても同じ（実はsetは順序付きなのでソートは不要）。setとvectorの相互変換はコンストラクタを用いると簡単
...
- 有向グラフの閉路判定は、トポロジカルソートができなければ閉路ありと判定可能
...
- 自分定義の構造体をvectorにする場合は、初期値を設定しないとエラーになる
