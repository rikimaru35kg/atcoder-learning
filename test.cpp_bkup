...
S op(S a, S b) {
    S c;
    c.w = a.w + b.w;
    c.d0 = merge(a.d0, b.d0, a.w, b.w);
    c.d1 = merge(a.d1, b.d1, a.w, b.w);
    return c;
}
S e() { return S({0,D({0,0,0}),D({0,0,0})}); }
using F = int;
S mapping (F f, S x) {
    if (f==0) return x;
    swap(x.d0, x.d1);
...
int main () {
    ...
    vector<S> init(N);
    rep(i, N) {
        if (Str[i]=='0') init[i] = S({1,D({1,1,1}),D({0,0,0})});
        else init[i] = S({1,D({0,0,0}),D({1,1,1})});
    }
