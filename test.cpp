...
struct S {
    ll a,b,c;
    S(ll a=0, ll b=0, ll c=0): a(a),b(b),c(c) {}
};
S op(S x, S y) { return S(x.a+y.a, x.b+y.b, x.c+y.c); }
S e() {return S();}
using F = ll;
S mapping(F f, S x) {
    if(f==-1) return x;
    ll w = x.a+x.b+x.c;
    if(f==0) return S(w,0,0);
    else if(f==1) return S(0,w,0);
    else return S(0,0,w);
}
...
void solve() {
    ...
    vector<S> init(N);
    rep(i, N) {
        if(P[i]==0) init[i] = S(1,0,0);
        else if(P[i]==1) init[i] = S(0,1,0);
        else if(P[i]==2) init[i] = S(0,0,1);
    }
    ...
    auto segprint=[&](){
        ...
        rep(i, sz) {
            auto mon = seg.get(i);
            fprintf(stderr, "(%lld,%lld,%lld) ", mon.a, mon.b, mon.c);
        }
        cerr<<endl;
    #endif
    };
    ...
    rep(i, Q) {
        LONG(t, l, r); --l;
        auto mon = seg.prod(l, r);
        if(t==1) {
            seg.apply(l, l+mon.a, 0);
            seg.apply(l+mon.a, l+mon.a+mon.b, 1);
            seg.apply(l+mon.a+mon.b, r, 2);
        } else {
            seg.apply(l, l+mon.c, 2);
            seg.apply(l+mon.c, l+mon.c+mon.b, 1);
            seg.apply(l+mon.c+mon.b, r, 0);
        }
    segprint();
    }
