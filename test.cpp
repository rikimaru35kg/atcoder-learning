...
void solve() {
    ...
    vvvvl dp(2, vvvl(K, vvl(K, vl(N+1, INF))));
    dp[0][N][N][0] = 0;

    auto dist=[&](ll i, ll j) -> ll {
        return abs(X[i]-X[j]);
    };
    auto gain=[&](ll i, ll j, ll t) -> ll {
        ll nt = dist(i,j) + t;
        if(nt<=T[j]) return 1;
        else return 0;
    };
    for(ll l=N; l>=0; --l) for(ll r=N; r<K && r-l<N; ++r) rep(n, N) rep(d, 2) {
        ll t = dp[d][l][r][n];
        if(t==INF) continue;
        ll i = l;
        if(d) i = r;
        if(l>0)   chmin(dp[0][l-1][r][n+gain(i,l-1,t)], t+dist(i,l-1));
        if(r<K-1) chmin(dp[1][l][r+1][n+gain(i,r+1,t)], t+dist(i,r+1));
    }

    ll ans = 0;
    rep(d, 2) for(ll l=N; l>=0; --l) for(ll r=N; r<K; ++r) rep(n, N+1) {
        ll t = dp[d][l][r][n];
        if(t==INF) continue;
        chmax(ans, n);
    }
