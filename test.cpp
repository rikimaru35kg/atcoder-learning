...
int main () {
    ...
    sort(all(xo)); sort(all(yo)); sort(all(xe)); sort(all(ye));

    auto calc=[&](vl &x) -> ll {
        ll n = SIZE(x);
        ll ret = 0;
        rep(i, n-1) {
            ret += (i+1)*(n-1-i)*(x[i+1]-x[i]);
        }
        return ret;
    };

    ll ans = 0; ans += calc(xo); ans += calc(yo); ans += calc(xe); ans += calc(ye);
    Out(ans);
