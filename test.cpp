...
struct S {
    ll s0, s1, n;
    S(ll s0=0, ll s1=0, ll n=0):s0(s0),s1(s1),n(n){}
};
S op(S a, S b) {
    if(a.n&1) return S(a.s0+b.s1, a.s1+b.s0, a.n+b.n);
    else return S(a.s0+b.s0, a.s1+b.s1, a.n+b.n);
}
S e() {return S();}
...
void solve() {
    ...
    ll z = 0;
    ll mod = 1e9;
    SegTree<S,op,e> seg(1e9+10);
    rep(i, Q) {
        LONG(y);
        ll x = ((y+z)%mod) + 1;
        seg.set_and_op(x, S(x, 0, 1));
        z = seg.all_prod().s0;
        Out(z);
    }
