...
int main () {
    ...
    sp.set(0, 0); sm.set(0, 0);
    vl dp(N, -INF);
    dp[0] = 0;
    auto upd=[&](segtree<S,op,e> &seg, ll i, ll x) {
        if(seg.get(i) >= x) return;
        seg.set(i, x);
    };
    auto dprint=[&](segtree<S,op,e> &seg){
    #ifdef __DEBUG
        rep(i, N) {
            cerr<< seg.get(i) <<' ';
        }
    ...
    rep(i, M) {
        ...
        // dprint(sm); dprint(sp);
        ll now = -INF;
        chmax(now, sp.prod(0, t) - C*t + p);
        chmax(now, sm.prod(t, N) + C*t + p);
        chmax(dp[t], now);
        de3(i, dp[t], now)
        upd(sp, t, dp[t]+C*t);
        upd(sm, t, dp[t]-C*t);
