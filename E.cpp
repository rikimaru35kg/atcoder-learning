...
struct WeightedUnionFind {
    ...
    vvl cable;
    WeightedUnionFind(long long n) : p(n,-1), num(n,1), diff(n), inf(n), cable(n) {}
...
void solve() {
    ...
    rep(i, M) {
        ...
        if(!uf.same(a,b)) {
            ...
            ll l = uf.leader(a);
            uf.cable[l].push_back(i);
        }
    }

    set<Pr> st;
    rep(i, N) if(i==uf.leader(i)) {
        st.emplace(SIZE(uf.cable[i]), i);
    }
    ll K = 0;
    vt3 ans;
    while(SIZE(st)>1) {
        ...
        st.erase(it1);
        st.erase(it2);
        ll ci = pop(uf.cable[i2]);
        auto [a,b] = edge[ci];
        ans.emplace_back(ci+1, a+1, i1+1);
        uf.merge(i1, i2);
        ll l = uf.leader(i1);
        st.emplace(SIZE(uf.cable[l]), l);
    }
    Out(K);
    for(auto [a,b,c]: ans) {
        printf("%lld %lld %lld\n",a,b,c);
